...
# Custom cd function that maintains a directory stack.
cd() {
    # Attempt to change directory using built-in cd.
    if builtin cd "$@" 2>/dev/null; then
        # Check if current directory is already in the stack.
        if ! dirs -v | grep -q "^[0-9]\+[[:space:]]\+${PWD}$"; then
            # Ensure stack doesn't exceed DIRSTACKSIZE.
            local stack_size=$(dirs -v | wc -l)
            if [ "$stack_size" -ge "${DIRSTACKSIZE:-10}" ]; then
                popd -n +"$DIRSTACKSIZE" >/dev/null 2>&1
            fi

            # Add current directory to stack silently.
            pushd -n "$PWD" >/dev/null 2>&1

            # Provide feedback.
            printf "Directory stack (%d entries):\n" "$((stack_size - 1))"
            dirs -v | tail -n +2 | head -n 9 | \
                awk '{printf "  %d: %s\n", NR, $2}'
        else
            printf "Current directory already in stack\n"
        fi
    else
