...
- ~C-w~: Move cursor forward one word.
- ~C-q~: Move cursor backward one word.

** Command History and Navigation

- ~<up>~: Navigate through command history.
- ~C-r~: Perform a reverse history search.
...
- ~C-y~: Yank (paste) the killed text.
...
- ~M-<left>~: Move the cursor backward one word.
- ~M-<right>~: Move the cursor forward one word.
- ~C-c~: Cancel the current command or foreground process.
