...
Normally, you can use ~cd -1~ to go to the *previous directory* in the stack, and so
...
# Check if the script is running in WSL (Windows Subsystem for Linux).
if [[ "$WSL_DISTRO_NAME" ]]; then
    # Alias for opening the Windows File Explorer.
    alias explorer='explorer.exe'

    # Aliases for opening files or URLs using Windows default applications.
