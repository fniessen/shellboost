...
autopushd() {
    ...
    if [[ ":$DIRSTACK:" != *":$old_dir:"* ]]; then
        printf "Bash - Pushing $old_dir onto the stack:\n"
        pushd "$old_dir"
    else
        printf "Bash - Directory already in stack, not pushing.\n"
    fi
}
...
cd() {
    if builtin cd "$@"; then
        if ! dirs -v | grep -q "$PWD"; then
            if [ $(dirs -v | wc -l) -ge $DIRSTACKSIZE ]; then
                popd -n +$DIRSTACKSIZE > /dev/null
            fi
            pushd . > /dev/null
            printf "Directory stack updated:\n"
            dirs -v
        fi
    fi
}
...
#+begin_src shell :tangle bin/rc-bash-specific-settings-shell-leuven-new
# Jump to a directory by index.
jumpd() {
    cd "$(dirs +$1 2> /dev/null)" || printf "Invalid index: %s\n" "$1"
}
