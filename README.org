...
__git_ps1() {
    ...
    if ! git diff --quiet --no-ext-diff; then
        dirty="${GIT_RESET_COLOR} ${GIT_DIRTY_COLOR}*dirty${GIT_RESET_COLOR}"
    fi

    if [[ -n "$(git diff --no-ext-diff --cached --quiet || echo '+')" ]]; then
        staged="${GIT_RESET_COLOR} ${GIT_STAGED_COLOR}+staged${GIT_RESET_COLOR}"
    fi

    if git rev-parse --verify --quiet refs/stash > /dev/null; then
        stash="${GIT_RESET_COLOR} ${GIT_STASH_COLOR}\$stash${GIT_RESET_COLOR}"
    fi

    if git ls-files --others --exclude-standard --error-unmatch -- ':/*' > /dev/null 2>&1; then
        untracked="${GIT_RESET_COLOR} ${GIT_UNTRACKED_COLOR}?untracked${GIT_RESET_COLOR}"
    fi
    ...
    if (( count_outgoing > 0 )); then
        outgoing="${GIT_RESET_COLOR} ${GIT_OUTGOING_COLOR}↑$count_outgoing${GIT_RESET_COLOR}"
    fi

    if (( count_incoming > 0 )); then
        incoming="${GIT_RESET_COLOR} ${GIT_INCOMING_COLOR}↓$count_incoming${GIT_RESET_COLOR}"
    fi

    local status_info="$dirty$staged$stash$untracked$outgoing$incoming"

    branch="${branch##refs/heads/}"
    printf -- " (${GIT_BRANCH_COLOR}${branch}${status_info}${GIT_RESET_COLOR})"
}
