...
To dynamically extend ~PATH~, we define a helper function that adds a directory
only if it exists and is not already present. This prevents duplicate entries
...
This ensures that executables installed in user-specific locations are available
from the command line. In particular, ~~/.local/bin~ is the default installation
directory for executables installed with ~pip --user~, such as ~xlsx2csv~.
...
The following aliases provide concise variants of the GNU ~ls~ command for common
directory inspection tasks.

Color output is enabled only when writing to a terminal. This preserves readable
output when a command is redirected to a file or passed through a pipeline.
...
cd() {
    ...
    if builtin cd "$@" 2>/dev/null; then
        ...
        # dirs -v | tail -n +2 | head -n 9 | sed 's_^ __'
    else
...
if [ -z "$ZSH_VERSION" ]; then
    printf "ERROR: This script requires Zsh. Aborting.\n" >&2
    return 1
fi
...
jumpd() {
    ...
    if ! [[ "$index" =~ ^[0-9]+$ ]]; then
        printf "jumpd: Index must be a non-negative number\n" >&2
    ...
    if builtin cd "$target_dir" 2>/dev/null; then
        ...
        printf "jumpd: Failed to cd to: %s\n" "$target_dir" >&2
...
XXX it's not really cleared neither in Bash nor in Zsh

#+begin_src shell :tangle bin/rc-zsh-specific-settings-shellboost
function zle-reset-terminal() {
  command reset
  zle redisplay
}
zle -N zle-reset-terminal

# C-l.
bindkey '^l' zle-reset-terminal
#+end_src

Does it delete command history?
...
if ! command -v git >/dev/null 2>&1; then
    printf "\033[1;37;41mERROR: 'git' command not found.\033[0m\n" >&2
fi
...
Messages sent to file descriptor ~2~ (*stderr*) are wrapped with ANSI escape
sequences so that they are displayed in red in the terminal.

#+begin_src shell :tangle bin/rc-zsh-specific-settings-shellboost
# Color stderr line by line, including a final unterminated line.
color_stderr_red() {
    emulate -LR zsh

    local red=$'\e[37;1;41m'
    local reset=$'\e[0m'
    local line
    ...
    while IFS= read -r line || [[ -n $line ]]; do
        ...
        [[ -t 2 ]] && zmodload zsh/system 2>/dev/null \
            && syswrite -o 2 ''
    done
}

# Color stderr in interactive shells when stderr is connected to a terminal.
...
#+begin_warning
This transforms stderr into an asynchronous pipeline:

stderr -> pipe -> color_stderr_red -> terminal

As a consequence, the relative ordering of stdout and stderr messages is no
longer guaranteed.  Error messages may appear later than expected, and prompts
displayed with commands such as ~read -p~ may be delayed, duplicated, or appear
out of order.

This behaviour has been observed with scripts such as ~git-switch-https-to-ssh~,
where stderr messages are displayed after an interactive prompt even though they
were emitted beforehand.
#+end_warning
